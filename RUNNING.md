...
163 checks covering port selection, groups and permissions, delegated work and approval, identity
and reclaim on reconnect, messaging, long-polling, delivery receipts, pausing, channel placement
and scoped listening, channel deletion, session reaping, the wire shapes the console reads,
ask/reply, error handling, and persistence across a restart. Expected output ends with
`163 passed, 0 failed`.
